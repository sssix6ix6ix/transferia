...
func (b *insertBuilder) Build() (InsertEvent, error) {
	...
	if err := errors.Join(b.errs...); err != nil {
		return nil, xerrors.Errorf("unable to build event: %w", err)
	}
	return b.newEvent()
}
...
func NewDefaultInsertBuilder(table abstract2.Table) InsertBuilder {
	keyPos := map[string]int{}
	...
	return &insertBuilder{
		table:       table,
		keyPos:      keyPos,
		vals:        make([]abstract2.Value, table.ColumnsCount()),
		errs:        nil,
		position:    nil,
		transaction: nil,
	}
