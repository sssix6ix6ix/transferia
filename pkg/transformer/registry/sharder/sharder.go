...
func init() {
	transformer.Register[Config](Type, func(cfg Config, lgr log.Logger, runtime abstract.TransformationRuntimeOpts) (abstract.Transformer, error) {
		...
		var columns transformer_filter.Filter
		isRandom := false
		if cfg.IsRandom {
			isRandom = true
		} else {
			columns, err = transformer_filter.NewFilter(cfg.Columns.IncludeColumns, cfg.Columns.ExcludeColumns)
			if err != nil {
	...
	if f.IsRandom {
		...
		for i := range item.TableSchema.Columns() {
			...
			if f.Columns.Match(currColumnName) {
				arrValues = append(arrValues, transformer_to_string.SerializeToString(fieldNameToVal[currColumnName], item.TableSchema.Columns()[i].DataType))
			}
		}
